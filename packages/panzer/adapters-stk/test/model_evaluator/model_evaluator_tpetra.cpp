...
namespace panzer {
  ...
  struct AssemblyPieces {
    ...
    RCP<TpetraLinObjFactoryType> t_lof;
  ...
  TEUCHOS_UNIT_TEST(model_evaluator, basic)
  {
    ...
    // Test a transient me
    {
      ...
      RCP<PME> me = Teuchos::rcp(new PME(ap.fmb,ap.rLibrary,ap.t_lof,p_names,p_values,Teuchos::null,ap.gd,build_transient_support,0.0));

      InArgs in_args = me->createInArgs();
      OutArgs out_args = me->createOutArgs();

      TEST_ASSERT(in_args.supports(Thyra::ModelEvaluatorBase::IN_ARG_x));
      TEST_ASSERT(in_args.supports(Thyra::ModelEvaluatorBase::IN_ARG_x_dot));
      TEST_ASSERT(in_args.supports(Thyra::ModelEvaluatorBase::IN_ARG_alpha));
      TEST_ASSERT(in_args.supports(Thyra::ModelEvaluatorBase::IN_ARG_beta));
      TEST_ASSERT(out_args.supports(Thyra::ModelEvaluatorBase::OUT_ARG_f));
      TEST_ASSERT(out_args.supports(Thyra::ModelEvaluatorBase::OUT_ARG_W_op));
  ...
  TEUCHOS_UNIT_TEST(model_evaluator, response)
  {
    ...
    typedef Thyra::VectorBase<RealType> VectorType;
    typedef Thyra::MultiVectorBase<RealType> MultiVectorType;
    typedef Thyra::SpmdVectorBase<RealType> SpmdVectorType;
    typedef Thyra::LinearOpBase<RealType> OperatorType;
    ...
    const double SOURCE_VALUE = .5;
    const double TEMPERATURE_VALUE = 5.;
        ...
        // Test a steady-state me
    {
      ...
      int rIndex = 0;
      {
        ...
        builder.linearObjFactory = ap.t_lof;
        ...
        rIndex = me->addResponse("INTEGRAND",blocks,builder);
      }
      ...
      out_args.set_g(rIndex,g);

      RCP<MultiVectorType> dgdp = Thyra::createMember(*me->get_g_space(rIndex)); 
      out_args.set_DgDp(rIndex,0,Thyra::ModelEvaluatorBase::Derivative<double>(dgdp));

      me->evalModel(in_args, out_args);
            out << "FUNC = " << std::endl;
      out << Teuchos::describe(*x,Teuchos::VERB_EXTREME) << std::endl;
            out << "RESIDUAL = " << std::endl;
      out << Teuchos::describe(*f,Teuchos::VERB_EXTREME) << std::endl;
            out << "RESPONSE = " << std::endl;
      out << Teuchos::describe(*g,Teuchos::VERB_EXTREME) << std::endl;
            out << "DGDP = " << std::endl;
      out << Teuchos::describe(*dgdp,Teuchos::VERB_EXTREME) << std::endl;
      ...
      rcp_dynamic_cast<Thyra::SpmdVectorBase<double> >(dgdp)->getLocalData(Teuchos::ptrFromRef(dgdp_data));

      TEST_FLOATING_EQUALITY(g_data[0],TEMPERATURE_VALUE*TEMPERATURE_VALUE*SOURCE_VALUE*SOURCE_VALUE*SOURCE_VALUE,1e-12);
      TEST_FLOATING_EQUALITY(dgdp_data[0],TEMPERATURE_VALUE*TEMPERATURE_VALUE*SOURCE_VALUE*SOURCE_VALUE*3.,1e-12);
...
//  }
                           ...
                           AssemblyPieces & ap)
  {
    ...
    auto bc_factory = Teuchos::rcp(new user_app::BCFactory);
    ap.bc_factory = bc_factory;
    ...
    ap.t_lof = Teuchos::rcp(new TpetraLinObjFactoryType(tComm.getConst(),Teuchos::rcp_dynamic_cast<panzer::DOFManager>(ap.dofManager)));
    linObjFactory = ap.t_lof;
    ...
    if(parameter_on)
       ...
       closure_models.sublist("solid").sublist("SOURCE_TEMPERATURE").set<double>("Value",.5);
    ...
    closure_models.sublist("solid").sublist("INTEGRAND").set<std::string>("Term Names","TEMPERATURE,TEMPERATURE,SOURCE_TEMPERATURE,SOURCE_TEMPERATURE,SOURCE_TEMPERATURE");
